...
class testContinuousEntanglement(unittest2.TestCase):
    ...
    def testEntangledOscillators(self):
        ...
        self.assertAlmostEqual(quad(lambda x1: np.conjugate(decompositions[0][1](np.array([x1])))*decompositions[0][1](np.array([x1])), -10, 10)[0], 1)
        self.assertAlmostEqual(quad(lambda x2: np.conjugate(decompositions[0][2](np.array([x2])))*decompositions[0][2](np.array([x2])), -10, 10)[0], 1)
